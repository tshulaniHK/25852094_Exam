...
I compute **Spearman $\rho$** between each year's top-25 names and the rankings 1, 2 and 3 years later.

> $\rho \approx 1$ means the same names stayed popular in the same order; $\rho \approx 0$ means the rankings shuffled completely.
...
persistence_df %>%
    ...
    labs(title = "Do Popular Baby Names Persist? Spearman Rank Correlation Over Time",
         ...
         x = NULL, y = expression(Spearman~rho), colour = "Lag") +
    theme_minimal(base_size = 13) +
    theme(legend.position = "top")
```
...
persistence_df %>%
    ...
    kable(caption = "Mean Spearman rho: Pre-1990 vs 1990 Onwards",
          booktabs = TRUE) %>%
...
- The decline is most visible at longer lags: the 3-year lag $\rho$ dropped from $\sim$0.93 (boys) and $\sim$0.85 (girls) pre-1990 to $\sim$0.84 and $\sim$0.82 post-1990.
...
top_surges %>%
    ...
    slice_head(n = 15) %>%
    kable(col.names = c("Name", "Gender", "Year", "Prev Year", "Surge Year",
                         "Change", "Pct Change"),
...
- **Nevaeh** (+2,125\% in 2001) is "heaven" spelled backwards --- a name essentially invented by musician Sonny Sandoval when he appeared on MTV and mentioned naming his daughter Nevaeh. It went from 53 births to over 1,000 in a single year.
...
billboard_baby_match %>%
    ...
    kable(col.names = c("Name", "Gender", "Chart Peak", "Name Surge Year",
                         "Billboard Weeks", "Prev Births", "Surge Births", "Pct Change"),
...
- **Whitney** is the clearest example: Billboard dominance led to a massive baby name surge.
...
tribble(
    ~Finding,                                                           ~Evidence,
    "Post-1990 names are less persistent",                              "Spearman rho drops for 1--3 year lags after 1990",
...
- **Monitor Billboard and streaming charts** --- artist first names influence baby naming within 1--3 years of peak fame.
- **Track new TV/streaming character names** --- unique fantasy names can emerge from zero to thousands within 2--3 years.
- **Do not bet on longevity** --- modern name trends last roughly 5--10 years at most; refresh toy character names every product cycle.
